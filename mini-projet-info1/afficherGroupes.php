...
<a class="navbar-brand animated bounce " id="animated-img1" href="index.php"><img id="logo" src="http://www.enicarthage.rnu.tn/assets/images/logo.png"alt="logo" width=45%/></a>
  ...
  <main role="main">
    <div class="jumbotron">
      <div class="container">
        ...
        <h1 class="display-2">Liste des groupes</h1>
    ...
    <div class="container">
      <div class="row">
        ...
        <button type="submit" id="me" class="btn  btn-block active" onclick="refresh()">Actualiser</button>
      </div>
    </div>
  ...
  <script>
    function deleteGroupe(id) {
      ...
      var url = `http://localhost/mini-projet-info1/auth-php-mysql/deleteGroupe.php?id=${id}`;
    ...
    function editGroupe(groupe) {
      var xmlhttp = new XMLHttpRequest();
      var url = `http://localhost/mini-projet-info1/editGroupeform.php?etd=${groupe}`;
      ...
      xmlhttp.onreadystatechange = function() { // alert(this.readyState+" "+this.status);
        if (this.readyState == 4 && this.status == 200) {
          ...
          // alert(this.responseText);
          console.log(this.responseText);
          window.location.href = `http://localhost/mini-projet-info1/editGroupeform.php?etd=${groupe}`;
          //console.log(this.responseText);
        }
      }
    }
    ...
    function refresh() {
      var xmlhttp = new XMLHttpRequest();
      var url = "http://localhost/mini-projet-info1/auth-php-mysql/afficherGroupe.php";
      ...
      function myFunction(response) {
        ...
        if (obj.success == 1) {
          ...
          out += " <tr><th>Id</th><th>Nom</th><th>Description</th><th>Edit</th><th>Delete</th></tr>";
          for (i = 0; i < arr.length; i++) {
            out += "<tr><td>" +
              ...
              "</td>" +
              "<td>" +
              `<button onClick=editGroupe(${arr[i].id_groupe})>Edit</button>` +
              "</td>" +
              "<td>" +
              `<button onClick=deleteGroupe(${arr[i].id_groupe})>delete</button>` +
              "</td></tr>";
          }
          out += "</table>";
